...
import (
	...
	"github.com/halladj/dis-log/internal/log"
...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		config := log.Config{}
		config.Raft.StreamLayer = log.NewStreamLayer(
			ln, nil, nil,
		)
		config.Raft.LocalID = raft.ServerID(
			fmt.Sprintf("%d", i),
		)
